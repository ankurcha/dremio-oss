...
public class TestDatasetProfiles extends BaseTestServer {
  ...
  private static Field bigInt(String name) {
    return new Field(name, new FieldType(true, MinorType.BIGINT.getType(), null, null), null);
  }

  private static Field varchar(String name) {
    return new Field(name, new FieldType(true, MinorType.VARCHAR.getType(), null, null), null);
  }

  private static Field dbl(String name) {
    return new Field(name, new FieldType(true, MinorType.FLOAT8.getType(), null, null), null);
  }

  private static Field map(String name, Field...children) {
    return new Field(name, new FieldType(true, MinorType.STRUCT.getType(), null, null), Arrays.asList(children));
  }

  private static Field list(String name, Field inner) {
    return new Field(name, new FieldType(true, MinorType.LIST.getType(), null, null),
