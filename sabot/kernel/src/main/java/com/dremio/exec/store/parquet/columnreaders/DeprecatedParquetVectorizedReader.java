      ...
      if (!isStarQuery()) {
        ...
        for (int i = 0; i < columnsFound.length; i++) {
          ...
          if ( ! columnsFound[i] && !col.equals(STAR_COLUMN)) {
            nullFilledVectors.add((IntVector)output.addField(new Field(col.getAsUnescapedPath(), new FieldType(true,
                    getArrowMinorType(MinorType.INT).getType(), null, null), null),
