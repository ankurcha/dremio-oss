...
public class ParquetTypeHelper {
  ...
  private static Optional<Field> toField(final Type parquetField, final SchemaDerivationHelper schemaHelper, boolean convertToStruct) {
    if (parquetField.isPrimitive()) {
      ...
      if (parquetField.isRepetition(REPEATED)) {
        Field listChild = new Field("$data$", new FieldType(true, field.getType(), null, null), field.getChildren());
        return Optional.of(new Field(field.getName(), new FieldType(true, new ArrowType.List(), null, null), Arrays.asList(listChild)));
      }
    ...
    if (OriginalType.LIST == complexField.getOriginalType() && LogicalListL1Converter.isSupportedSchema(complexField)) {
      ...
      subField = subField.map(sf -> new Field("$data$", new FieldType(true, sf.getType(), null, null), sf.getChildren()));
      return subField.map(sf -> new Field(complexField.getName(), new FieldType(true, new ArrowType.List(), null, null), Arrays.asList(new Field[] {sf})));
      ...
      if (complexField.getFieldCount() == 1) {
        ...
        if (complexField.isRepetition(REPEATED)) {
          subField = subField.map(sf -> new Field("$data$", new FieldType(true, new ArrowType.Struct(), null, null), Arrays.asList(new Field[] {sf})));
          return subField.map(sf -> new Field(complexField.getName(), new FieldType(true, new ArrowType.List(), null, null), Arrays.asList(new Field[] {sf})));
        } else {
          return subField.map(sf -> new Field(complexField.getName(), new FieldType(true, new ArrowType.Struct(), null, null), Arrays.asList(new Field[] {sf})));
        }
      }
      throw UserException.unsupportedError()
    ...
    if (OriginalType.MAP == complexField.getOriginalType() && !convertToStruct) {
      ...
      if (complexField.isRepetition(REPEATED)) {
        subField = subField.map(sf -> new Field("$data$", new FieldType(true, sf.getType(), null, null), sf.getChildren()));
        subField = subField.map(sf -> new Field(repeatedField.getName(), new FieldType(true, new ArrowType.List(), null, null), Arrays.asList(new Field[] {sf})));
        subField = subField.map(sf -> new Field("$data$", new FieldType(true, new ArrowType.Struct(), null, null), Arrays.asList(new Field[] {sf})));
        return subField.map(sf -> new Field(complexField.getName(), new FieldType(true, new ArrowType.List(), null, null), Arrays.asList(new Field[] {sf})));
      } else {
        subField = subField.map(sf -> new Field("$data$", new FieldType(true, sf.getType(), null, null), sf.getChildren()));
  ...
  private static Optional<Field> toComplexField(GroupType complexField, ArrowType arrowType, SchemaDerivationHelper schemaHelper, boolean convertToStruct) {
    ...
    Field field = new Field(complexField.getName(), new FieldType(true, arrowType, null, null), subFields);
    if (complexField.isRepetition(REPEATED)
      && !convertToStruct
